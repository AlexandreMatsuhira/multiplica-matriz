#!/usr/bin/env python3
"""
Script para executar multiplicação paralela local (multi-core)
"""
import argparse
import sys
...
from multiplication import multiply_parallel_local
from config import DEFAULT_BLOCK_SIZE


def main():
    parser = argparse.ArgumentParser(
        description='Multiplicação paralela local de matrizes'
    )
    parser.add_argument(
        '--matA',
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help='Número de núcleos a usar (padrão: todos disponíveis)'
    )
    parser.add_argument(
        '--block-size',
        type=int,
    ...
    num_cores = args.cores if args.cores else max_cores
    num_cores = min(num_cores, max_cores)
    
    print("=" * 70)
    print("MULTIPLICAÇÃO PARALELA LOCAL DE MATRIZES")
    print("=" * 70)
    ...
    print(f"\nExecutando multiplicação paralela local...")
    print(f"  Núcleos disponíveis: {max_cores}")
    print(f"  Núcleos utilizados: {num_cores}")
    print(f"  Block size: {args.block_size}")
    print(f"  Usando shared memory para comunicação entre processos")
    
    try:
        matC, elapsed_time = multiply_parallel_local(
            matA, matB, num_cores, args.block_size
        )
    ...
    if benchmark_data:
        stats["Parallel_Local"]["benchmark"] = benchmark_data
